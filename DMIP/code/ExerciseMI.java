	...
	public double calculateMutualInformation(Grid2D ref, Grid2D mov){
		...
		for (int i = 0; i < histSize; i++) {
			...
			for (int j = 0; j < histSize; j++){
				...
				if(jointHistogram.getElement(i, j) != 0) {
					...
					entropy_jointHisto -= jointHistogram.getElement(i, j)*Math.log(jointHistogram.getElement(i, j))/Math.log(2);//TODO
				}
			}
		}
		...
		for(int i = 0; i < histSize; i++){
			...
			if(histo1.getElement(i) != 0){
				...
				entropy_histo1 -= histo1.getElement(i)*Math.log(histo1.getElement(i))/Math.log(2); //TODO
			}
			
			if(histo2.getElement(i) != 0){
				
				// calculate entropy for histogram 2 (hint: use logarithm base 2 and use the correct sign)
				entropy_histo2 -= histo2.getElement(i)*Math.log(histo2.getElement(i))/Math.log(2); //TODO
			}
		}
	...
	public SimpleMatrix calculateJointHistogram(Grid2D im1, Grid2D im2, int histSize){
		...
		for (int i = 0; i < imWidth; i++) {
			...
			for (int j = 0; j < imHeight; j++) {
				...
				jH.addToElement(value_ref,value_mov,1);//TODO
			}
		}
		
		// divide by the number of elements in order to get probabilities
		jH.divideBy(imWidth*imHeight);//TODO
		...
		for(int i = 0; i < numRows; i++) {
			...
			for(int j = 0; j < numCols; j++) {
				...
				if (sumRows) {
					...
					hist.addToElement(j, jH.getElement(i, j));//TODO
				}
				else {
					
					// sum up over the columns
					hist.addToElement(i, jH.getElement(i, j));//TODO
				}				
			}
		}
