...
public class ExerciseUM {
	...
	public Grid2D homomorphicFiltering(Grid2D grid, int kernelSize){
		...
		for (int i = 0; i < corrected.getHeight(); i++){
			for (int j = 0; j < corrected.getWidth(); j++){
				...
				corrected.setAtIndex(j, i, tempVal*corrected.getAtIndex(j, i));// TODO: apply the bias field correction for the multiplicative model
			}
		}
	...
	public Grid2D frequencyCorrection(Grid2D grid, double beta, double sigma){
		...
		for (int k = 0; k < fftimage.getHeight(); k++){
			for (int l = 0; l < fftimage.getWidth(); l++){	
				...
				float val = (float) (1.0f-beta*Math.exp(-(fk*fk+fl*fl)/(2.0f*sigma*sigma)));  // TODO
				fftimage.setRealAtIndex(l, k, fftimage.getRealAtIndex(l, k)*val);// TODO: setRealAtIndex for fftimage
				fftimage.setImagAtIndex(l, k, fftimage.getImagAtIndex(l, k)*val);// TODO: setImageAtIndex for fftimage
			}
		}
	...
	public Grid2D frequencyCutoff(Grid2D grid,double cutoff) {
		...
		Grid2D returnGrid = new Grid2D(highPass(grid,cutoff)); // TODO: apply a hard frequency cut-off (high pass filter), ...
		//returnGrid; // ... look for the appropriate class method in this file
