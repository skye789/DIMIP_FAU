...
def box_filter(img, r):
    ...
    result = box_filter_rows(box_filter_columns(img, r), r)
...
def box_filter_columns(img, r):
    ...
    result[r + 1: rows - r, :] = integral_image_cols[2 * r + 1: rows, :] - integral_image_cols[0: rows - 2 * r - 1,
                                                                           :]  # TODO: implement the correct steps here
    # image boundary (bottom)
    result[rows - r: rows, :] = np.tile(integral_image_cols[rows - 1, :], [r, 1]) - integral_image_cols[
                                                                                    rows - 2 * r - 1: rows - r - 1,
                                                                                    :]  # TODO: implement the correct steps here
...
def box_filter_rows(img, r):
    ...
    # regular areas (box fully overlapping with image)
    result[:, r + 1: columns - r] = integral_image_rows[:, 2 * r + 1: columns] - integral_image_rows[:,
                                                                                 0: columns - 2 * r - 1]  # TODO: implement the correct steps here
    # image boundary (right)
    result[:, columns - r: columns] = np.tile(integral_image_rows[:, columns - 1], [r, 1]).T - integral_image_rows[:,
                                                                                               columns - 2 * r - 1: columns - r - 1]  # TODO: implement the correct steps here
...
def guided_filter(i, g, r, epsilon):
    ...
    var_i = corr_i - mean_i ** 2  # TODO: implement the correct steps here
