...
public class ExerciseFB {
	...
	void initParameters() {
		...
		halfFanAngle = (float) Math.atan(detectorLength/2/focalLength); //TODO: determine the half fan angle such that the complete fan covers the detector exactly (hint: trigonometry) 
		maxBeta = (float) (Math.PI+2*halfFanAngle); //TODO: compute the short scan minimum range using the value of halfFanAngle
	...
	public Grid2D applyCosineWeights(Grid2D sino) {
		...
		for(int i=0; i < detectorPixels; ++i){
			...
			float t = i*detectorSpacing - detectorLength/2.f + 0.5f*detectorSpacing; //TODO: compute center of the current detector pixel (zero is center of detector)
			cosineKernel.setAtIndex(i, (float) (focalLength/Math.pow(t*t+focalLength*focalLength, 0.5))); //TODO (hint: use t and focalLength)
		}
	...
	public Grid2D backproject(Grid2D sino, int[] recoSize, float[] spacing) {
		...
		for(int p = 0; p < numProjs; p++) {
			...
			final PointND detBorder = new PointND(offset*cosBeta, offset*sinBeta, 0.f);  //TODO
			
			//compute direction vector along the detector array (towards detector center)
			SimpleVector dirDet = new SimpleVector(-detBorder.getCoordinates()[0],-detBorder.getCoordinates()[1],0); //TODO
			if (dirDet == null)
	...
	public float getWorldCoordinate(int imCoordinate, int dim, float spacing) {
		...
		float wCoordinate = (float) (imCoordinate-0.5*(dim-1))*spacing;  //TODO
	...
	public float distanceWeight(PointND reconstructionPointWorld, float beta) {
		...
		double x=reconstructionPointWorld.get(0);
		double y=reconstructionPointWorld.get(1);
		float radius = (float) reconstructionPointWorld.getAbstractVector().normL2(); //TODO
		float phi = (float) (Math.atan2(y, x)+Math.PI/2); //TODO
