...
public class ExerciseGeoU {
	...
	public void getLatticePoints (Grid2D distortedImage){
		...
		for(int i = 0; i < nx; i++){
			for(int j = 0; j < ny; j++){
				...
				Xu.setElementValue(j, i, xprime.getAtIndex(xx,yy));//TODO: fill matrix Xu
				Yu.setElementValue(j, i, yprime.getAtIndex(xx,yy));//TODO: fill matrix Yu
				Xd.setElementValue(j, i, x.getAtIndex(xx,yy));//TODO: fill matrix Xd
				Yd.setElementValue(j, i, y.getAtIndex(xx,yy));//TODO: fill matrix Yd
			}
		}
	}
	...
	public void calcPolynomial(int degree, SimpleMatrix Xd){
		...
		numCoeff = (int) ((d+1)*(d+2)/2); //TODO
		
		// number of correspondences: numCorresp
		numCorresp =  Xd.getRows()*Xd.getCols(); //TODO
		...
		for(int r = 0; r < numCorresp; r++){
			...
			for(int k = 0; k <= degree; k++){
				for(int l = 0; l <= (degree-k); l++){
					...
					A.setElementValue(r, cc, Math.pow(XdVector.getElement(r), k)*Math.pow(YdVector.getElement(r), l));// TODO: fill matrix A
					cc++;
	...
	public void computeDistortionCoeff(SimpleMatrix A, SimpleVector XuVector, SimpleVector YuVector){
	    ...
	    A_pseudoinverse = SimpleOperators.multiplyMatrixProd(svd.getV(),svd.getreciprocalS()); // TODO
	    A_pseudoinverse = SimpleOperators.multiplyMatrixProd(A_pseudoinverse,svd.getU().transposed());
